...
소비 기록은 카메라 뷰파인더의 셔터, 앨범 또는 사진 없음 quick action에서 시작한다.

사용자는 홈의 오늘의 소비 캔버스에서 기록하기 또는 탭바 추가를 누른 뒤 카메라 화면에서 사진을 촬영하거나 앨범에서 이미지를 선택하거나 사진 없이 기록한다. 이후 카테고리를 선택하고 가격을 입력하면 기록이 완료된다.
...
라이브 기록은 사진 선택을 낮은 시트로 받은 뒤 카테고리와 금액을 넓은 시트에서 함께 받는다. 긴 폼을 만들지 않고 카테고리 그리드와 금액 키패드만 둔다. visual CI는 Figma staged 카테고리/금액 시트를 유지한다.
