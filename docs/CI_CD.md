...
TestFlight CD는 `.github/workflows/ios-testflight.yml`이 소유한다. 성공한 `main` **push** iOS CI 또는 `main`의 `workflow_dispatch`에서만 실행하고 `ios-testflight` environment secret만 사용한다. archive는 로컬 Development 인증서 없이 만들고, export가 App Store Connect API key로 Apple Distribution cloud signing만 적용한다. PR에서 성공한 iOS CI는 TestFlight를 시작하지 않는다. PR과 `ios-ci.yml`에는 App Store Connect API key를 넣지 않는다. Bundle ID는 `com.ansandy.moneysnap`이다. Sign in with Apple runtime `.p8`은 서버 `server-development` secret이며 TestFlight 업로드 키와 분리한다. 레포는 분리하지 않는다.
